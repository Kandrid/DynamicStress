...
public final class DynamicStress extends JavaPlugin {
    ...
    boolean isInSight(Player player, Entity entity) {
        ...
        final double angle = Math.toDegrees(pVector.angle(eVector));//Math.abs(Math.toDegrees(Math.acos((eVector.getX() * pVector.getX() + eVector.getZ() * pVector.getZ()) / (eVector.length() * pVector.length()))));
